...
class ALEExperiment(object):
    ...
    def run_epoch(self, epoch, num_steps, testing=False):
        ...
        self.logger.info("\n============= New " + prefix + " epoch ===============")
        self.logger.info("Total runtime: {:.2f} min".format((self.start_time - time.time()) / 60))

        while steps_left > 0:
            #prefix = "testing" if testing else "training"
            if (self.should_log):
                self.logger.info(prefix + " epoch: " + str(epoch) + " steps_left: " +
                             str(steps_left))
    ...
    def run_episode(self, max_steps, testing):
        ...
        while True:
            ...
            if terminal or num_steps >= max_steps:
                self.should_log = self.agent.end_episode(reward, max_steps, terminal)
                break
