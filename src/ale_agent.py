...
class NeuralAgent(object):
    ...
    def __init__(self, q_network, epsilon_start, epsilon_min,
                 epsilon_decay, replay_memory_size, exp_dir,
    ...
    def _open_results_file(self):
        self.logger.info("OPENING " + self.exp_dir + '/results.csv')
    ...
    def end_episode(self, reward, max_steps, terminal=True):
        """
        ...
        if self.testing:
            ...
            # Only log every 10000 steps
            should_log = (max_steps % 10000 < 1000)
            # print("max_steps: ", max_steps)
            # print(max_steps % 10000)
            # print(should_log)
            # print(self.episode_no)
            # print("-----------------")

            if (should_log):
                # print("logging episode", self.episode_no)
                total_time = time.time() - self.start_time
                t = time.strftime("%H:%M:%S")
                self.logger.info("\n "+ t + " --------------- Episode " + str(self.episode_no) + "---------------")
                self.logger.info("Episode runtime: {:.1f} sec".format(total_time))

                # Print moving average of steps/second
                rho = 0.98
                self.steps_sec_ema *=rho
                self.steps_sec_ema += (1.0 - rho) * (self.step_counter/total_time)
                self.logger.info("steps/second: {:.2f}, avg: {:.2f}".format(self.step_counter/total_time, self.steps_sec_ema))

            if self.batch_counter > 0:
                self._update_learning_file()
                if (should_log):
                    self.logger.info("average loss: {:.4f}".format(\
